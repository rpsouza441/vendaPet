...
<fmt:message key="despesa.lista.title" var="title" />
...
<!-- Content Header (Page header) -->
    <section class="content-header">
      <h1>
           <fmt:message key="despesa.lista.h1" />
      </h1>
       <ol class="breadcrumb">
    ...
    <div class="row">
      ...
      <div class="col-xs-12">
          <div class="box">
            ...
            <!-- /.box-header -->
            <div class="box-body">
            <form:form action="${s:mvcUrl('DAPC#listaComSearch').build() }"
				    ...
				    <div class="col-xs-6">
						...
						<a  rel="tooltip" class="btn btn-success btn-block" id="edit_event"  
						 href="<s:url value='/despesa/cadastro' />">
							 <span class="glyphicon glyphicon-plus pull-left"></span> 
							   <fmt:message key="despesa.adicionar" />
						</a>
				    ...
				    <div class="col-xs-6">
						<div class="form-group">
						      ...
						      <fmt:message key="despesa.lista.searchPlaceHolder" var="searchPlaceHolder" />
                ...
                <h5>
												<i class="icon fa fa-ban"></i> <fmt:message
													key="despesa.produto.mensagem.naoEcontrado" />
											</h5>
              </div>
				            </c:if>
             ...
             <div class="table-responsive">
              <table id="example2" class="table table-bordered table-hover">
                <thead>
	                <tr>
	                  <th><fmt:message key="despesa.lista.nome" /></th>
	                  <th><fmt:message key="despesa.lista.dataVencimento" /></th>
	                  <th><fmt:message key="despesa.lista.dataPagamento" /></th>
	                  <th><fmt:message key="despesa.lista.valor" /></th>
	                   <th></th>
	                </tr>
                </thead>
                <tbody>
                ...
                <tr>
                  <td>${despesa.nome }</td>
                  <td>
	                  <fmt:formatDate value="${despesa.dataVencimento.time }"  
	                  pattern="dd/MM/yyyy" />
                  </td>
                  <td>
	                  <fmt:formatDate value="${despesa.dataPagamento.time }"  
	                  pattern="dd/MM/yyyy" />
                  </td>
                  <td>${despesa.valor }</td>
                  <td width="10%" class="td-actions text-middle">
						<a  rel="tooltip" class="btn btn-success"    
						href="<s:url value='/despesa/editarDespesa/${despesa.id}' />">
							 <i	class="fa  fa-pencil"></i> 
						</a>
						<a  rel="tooltip" class="btn btn-danger"    
						href="<s:url value='/despesa/excluirDespesa/${despesa.id}' />">
